...
struct PetrolStationView: View {
    @Environment(\.presentationMode) private var presentationMode
    @StateObject private var viewModel = PetrolStationViewModel()
    ...
    private var searchBar: some View {
        HStack(spacing: 10) {
            ...
            TextField("", text: $viewModel.searchText, prompt:
                Text("Search for a location or station...")
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 14) {
                ForEach(viewModel.filteredStations) { station in
                    PetrolStationCard(station: station)
                }
            }
        }
    }
...
private struct PetrolStationCard: View {
    let station: PetrolStation
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // Fuel types
            HStack(spacing: 6) {
                ForEach(station.fuelTypes, id: \.self) { fuel in
                    Text(fuel)
...
#Preview {
    NavigationView { PetrolStationView() }
}
