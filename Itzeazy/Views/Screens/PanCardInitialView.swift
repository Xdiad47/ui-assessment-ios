...
struct PanCardInitialView: View {
    @Environment(\.presentationMode) private var presentationMode
    @StateObject private var viewModel = PanCardInitialViewModel()
    ...
    private var heroContent: some View {
        VStack(spacing: 0) {
            VStack(spacing: 12) {
                Text("Hassle free PAN\ncard service")
                    .font(Font.custom("PlusJakartaSans-ExtraBold", size: 36))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.center)
                    .lineSpacing(4)

                Text("We make PAN card process faster")
