...
struct BirthCertInitialView: View {
    @Environment(\.presentationMode) private var presentationMode
    @StateObject private var viewModel = BirthCertInitialViewModel()
    ...
    var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .top) {
                ...
                NavigationLink(
                    destination: BirthCertDetailView(
    ...
    private var heroContent: some View {
        VStack(spacing: 0) {
            VStack(spacing: 12) {
                Text("Hassle free birth\ncertificate service")
                    .font(Font.custom("PlusJakartaSans-ExtraBold", size: 36))
                    .foregroundColor(.white)
                    .multilineTextAlignment(.center)
                    .lineSpacing(4)

                Text("We make birth certificate process faster")
