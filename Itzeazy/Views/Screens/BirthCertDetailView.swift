...
struct BirthCertDetailView: View {
    @Environment(\.presentationMode) private var presentationMode
    @EnvironmentObject private var authGate: AuthGateController
    @StateObject private var viewModel = BirthCertDetailViewModel()
    @StateObject private var webLoginVM = WebLoginViewModel()
    @StateObject private var disclaimerGate = GovDisclaimerGate(serviceKey: "birth_cert")
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        if viewModel.isLoading {
                            ...
                            HStack(spacing: 16) {
                                ...
                                if !viewModel.visitRequired.isEmpty {
                                    GovFeeMetricCard(icon: "visit_rto", label: "VISIT REQUIRED", value: viewModel.visitRequired)
                                }
                        ...
                        Button(action: {
                            ...
                            let url = "https://itzeazy.in/order/birth-certificate-\(location.lowercased())"
                            webLoginVM.generateToken(urlString: url, title: "Birth Certificate")
                        }) {
                            ZStack {
                        ...
                        GovDisclaimerJustificationCard(
                            text: "Itzeazy fee covers document verification, application filing " +
                                "guidance, follow-up with the municipal office, and doorstep support."
                        )
                        .padding(.horizontal, 24)
                        .padding(.bottom, 32)
...
#Preview {
    BirthCertDetailView(location: "Bangalore", service: "New Birth Certificate")
}
